...
一時的にsync出来てない状態に強い。タイムスタンプで別ファイルになるのでconflictしないし、最終的には時系列的に並ぶ。
これはスマホとノートPCを[[Syncthing]]でつないでいる場合に結構重要。
...
ページを作る前にどこに作るのかを考える必要がある。結構面倒。たまに良い場所が無かったりする。
...
今回は、[[てきすとTL]]で雑多に書いたものを適当に選んでTeFWikiのどれかのファイルにexport出来る機能を[[guash]]と[[htmnix]]を組み合わせて[[zx]]で作ってみた。しばらく使って感想を追記したい。