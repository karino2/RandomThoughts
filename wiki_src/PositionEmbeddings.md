...
なお、positional embedding無しでもそこそこスコアはでているので、これが無いと全然駄目でも無さそう。BoWになるはずなんだが。論文のテーブル4に以下のスコアがある。
...
sourceの方が影響はでかそう。これはtargetの方は現在のトークンと一つ前のトークンという関係自体はモデル構造に入るため、ある程度は重要な位置的なものは織り込めるからかな。
...
上のcolabで一緒にやっている。16、これは256のルートなので、つまりd_model/2のルートになっている。
これと合わせるために式3.4ではトークン側のembeddingにルートdを掛けている（なんでルート2で割らないのかは不明だが）。