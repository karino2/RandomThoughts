...
マルチヘッドのQ, K, Vの計算はd/8を8個出力するので結局同じ計算量になるのだが、そもそもd/8はオーダーとしてはdになってしまう。512程度でビッグO記法はどうなんだ、という話でもある。

なお、Q, K, VとWとの積は、Qの方はアテンションの種類によって1かnかの違いがあるが、KとVは行列との積がn個あると解釈して、$O(nd^2)$となる。
Qは
