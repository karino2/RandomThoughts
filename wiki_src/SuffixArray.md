...
かなりトリッキーな性質をいろいろ使うので、細かく理解しないと本当の事は分からないが、鍵となる部分を書いておく（書いておかないとすぐ完全に忘れてしまうので）。

- 基本的にはバケットソート（先頭の文字で分類したセット）の中をどうソートするかという話になる
- LMS Substringがソートされていれば、LMSの一つ左がL-typeなものは順番にでてくる（順番に挿入していくだけでソートになる）
