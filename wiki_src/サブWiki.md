...
## 今だったら単に複数ディレクトリのローカルwikiにする 2026-07-31 (金)

サブWikiのディレクトリで独立している必要がある、というのは全く正しかったと思っている。ネームスペースとかでは駄目で、サブWiki間ではリンクを貼れないようにする方が良い、というのは全く正しかった。

一方で親子関係は要らないな、と思うので、
単に複数のWikiサイトを持てるローカルWiki、とした方がコードも単純で他人からも分かりやすくて良かったな、と思う。
今作り直すならそうする。

ただ初回起動時の挙動を結構変える必要があるし、既存コンテンツの影響もあるので今から変える気は起こらない。
まぁユーザーはそんな居ないので変えてしまっても問題は無いとは思うが。

結局ルートのWikiは使わなくなるので、実質的には複数のWikiが持てるローカルWikiとして使っている訳で、現状でもそう使うのに何も不便は無いが、
それを理解するまでの分かりにくさがいまいちだったな、と思う。

## git-wikiでの複数ディレクトリ対応

サブWikiごとに別のレポジトリを持つのは面倒なので、[[GitWiki]]の一つのレポジトリに複数のサブディレクトリを掘ってそれぞれにサブWikiを置きたい。
